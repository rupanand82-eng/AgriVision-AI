...
app.post('/api/analyze-crop', async (req, res) => {
  try {
    ...
    try {
      ...
      res.json({
        diseaseName: "Leaf Spot / Nutrient Stress (Fallback Mode)",
        confidence: 85,
        symptoms: "[Notice: Gemini API is under quota limits.] The plant leaf presents light chlorotic symptoms with small circular spots active across lower segments.",
...
app.post('/api/analyze-soil', async (req, res) => {
  try {
    ...
    try {
      ...
      res.json({
        suitableCrops: `[API Quota Fallback] Highly resilient selections: Maize, Legumes (Beans/Peas), Sweet Potatoes, and deep alfalfa. Under pH ${ph || 6.5} and moisture content ${moisture || 40}%, these crops maximize nutrient utilization on ${soilType || 'Loam'} soils in ${location || 'this climate'}.`,
...
app.post('/api/irrigate', async (req, res) => {
  try {
    ...
    try {
      ...
      res.json({
        waterRequirement: `[API Quota Fallback] Target: 4.2 - 5.8 Liters per square meter daily. Under crop '${crop || 'Standard crop'}', moisture level ${soilMoisture || 30}% with ${temperature || 28}°C air outlines minor hydric stress risks.`,
...
app.post('/api/environmental-analysis', async (req, res) => {
  try {
    ...
    try {
      ...
      res.json({
        insights: `[API Quota Fallback] Ecological indices for '${location || 'your region'}' report stable environmental parameters. Ambient Air (${airQuality || 'MODERATE'}) and Hydric Resources (${waterQuality || 'SATISFACTORY'}) suggest low direct acidification risks. Trace pesticide vectors should be proactively buffered.`,
